...
import { colors, filterTasksByStatus } from '@/data/dataTasks'
...
export default function Home() {
  const { statuses, tasks, createStatus, createTask, updateTask, moveTask } =
    useStatuses()
      ...
      <section className="w-full flex flex-col gap-4 p-2 rounded-xl px-10 mx-auto">
        ...
        >
          <div className="flex items-center justify-between w-full">
            <TabsList className="flex p-1.5 h-auto self-start w-fit">
              <TabsTrigger
                value="kanban"
          ...
          <TabsContent value="kanban">
            <ScrollArea className="pb-4 focus:outline-none">
              <div className="flex gap-2">
                {statuses.map((status) => {
                  const filteredTasks = filterTasksByStatus(tasks, status.id)
                  return (
                    <Column
                      key={status.id}
                      status={status}
                      tasks={filteredTasks}
                      onDropTask={(taskId) =>
                        moveTask(parseInt(taskId), status.id)
                      }
                      onUpdateTask={updateTask}
                      onCreateTask={() => createTask(status.id)}
                    />
                  )
                })}
              </div>
